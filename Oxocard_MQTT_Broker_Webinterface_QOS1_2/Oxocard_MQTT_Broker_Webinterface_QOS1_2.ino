...
//
// Hardware: Oxocard Connect
//
// Letzte Änderung: 01.08.2025
// Adaption: 20.08.2025 Claus Kühnel info@ckuehnel.ch
// =================================================================================================
#include <WiFi.h>
#include <WebServer.h>
...
void setup() {
    ...
    tft.drawString("MQTT Broker", 120,28);
    ...
    tft.drawString("Port:", 0, 95); 
    tft.drawNumber(mqttPort, 80, 95); 
...
void handlePublish(MQTTClientState& clientState, byte header, int remainingLength) {
    ...
    tft.drawString("Received:", 0,140);
    tft.fillRect(0, 160, 240, 240, TFT_DARKGREY);
    tft.drawString("T: ", 0, 160); tft.drawString(topic.c_str(), 30, 160);
    tft.drawString("C: ", 0, 180); tft.drawString(payload.c_str(), 30, 180);
